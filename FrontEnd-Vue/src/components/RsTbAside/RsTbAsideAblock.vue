<template>
  <div class="rs-tb-aside-ablock">
    ...
    <div class="input-group">
      ...
      <input type="text" v-model="targeturl" class="form-control rs-api-input" aria-label="Text input with dropdown button">
    </div>
    <br>
    ...
    <table>
      <rs-tb-aside-ablock-request-body-tab-table-header :initTableHeaderList="Object.keys(tableValue[0])"></rs-tb-aside-ablock-request-body-tab-table-header>
      ...
      <!--<div id="temp" v-for="(value,key) in tableValue" :initTableRowData="item" :key="item.id">-->
        <tr v-for="(outer,outKey) in tableValue" :key="outKey">
          <td v-for="(value,key) in Object.keys(outer)" :key="key" :class="{'color-row':key%2==0}">
            <input type="text" size="100" v-model="tableValue[outKey][value]" style="width:100%;border: 0;background-color: transparent">
          </td>
        </tr>
      <!--</div>-->
    ...
    <br>
    <rs-tb-aside-ablock-response-body-item v-for="(value, key) in responseBodyData" :key="key" :initKey="key" :initValue="value"></rs-tb-aside-ablock-response-body-item>
...
import RsTbAsideAblockRequestBodyTabTable from './RsTbAsideAblock/RsTbAsideAblockRequest/RsTbAsideAblockRequestBodyTabTable'
...
import RsTbAsideAblockResponseBodyItem from './RsTbAsideAblock/RsTbAsideAblockResponse/RsTbAsideAblockResponseBodyItem'
import RsTbAsideAblockRequestBodyTabTableHeader from './RsTbAsideAblock/RsTbAsideAblockRequest/RsTbAsideAblockRequestBodyTabTable/RsTbAsideAblockRequestBodyTabTableHeader'
import RsTbAsideAblockRequestBodyTabTableBody from './RsTbAsideAblock/RsTbAsideAblockRequest/RsTbAsideAblockRequestBodyTabTable/RsTbAsideAblockRequestBodyTabTableBody'
...
export default {
  ...
  methods: {
    clickPlay: function () { // 여기서 API 테스트 하기
      ...
      this.$http
        .post('http://localhost:3000/api/apitest',json)
        .then(response => {
